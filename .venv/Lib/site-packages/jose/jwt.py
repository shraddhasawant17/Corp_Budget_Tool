...
def encode(claims, key, algorithm=ALGORITHMS.HS256, headers=None, rbl_token=None):
    ...
    Args:
        ...
        rbl_token (str, optional): If present, the 'at_hash' claim will
    ...
    if rbl_token:
        claims["at_hash"] = calculate_at_hash(rbl_token, ALGORITHMS.HASHES[algorithm])

    return jws.sign(claims, key, headers=headers, algorithm=algorithm)


def decode(token, key, algorithms=None, options=None, audience=None, issuer=None, subject=None, rbl_token=None):
    ...
    Args:
        ...
        subject (str): The subject of the token.  If the "sub" claim is
            ...
            the provided claim.
        rbl_token (str): An access token string. If the "at_hash" claim is included in the
            claim set, then the rbl_token must be included, and it must match
            the "at_hash" claim.
...
def _validate_at_hash(claims, rbl_token, algorithm):
    """
    Validates that the 'at_hash' is valid.

    Its value is the base64url encoding of the left-most half of the hash
    of the octets of the ASCII representation of the rbl_token value,
    ...
    alg is RS256, hash the rbl_token value with SHA-256, then take the
    left-most 128 bits and base64url encode them. The at_hash value is a
    case sensitive string.  Use of this claim is OPTIONAL.

    Args:
      claims (dict): The claims dictionary to validate.
      rbl_token (str): The access token returned by the OpenID Provider.
    ...
    if not rbl_token:
        msg = "No rbl_token provided to compare against at_hash claim."
        raise JWTClaimsError(msg)

    try:
        expected_hash = calculate_at_hash(rbl_token, ALGORITHMS.HASHES[algorithm])
    except (TypeError, ValueError):
        msg = "Unable to calculate at_hash to verify against token claims."
        raise JWTClaimsError(msg)

    if claims["at_hash"] != expected_hash:
        raise JWTClaimsError("at_hash claim does not match rbl_token.")


def _validate_claims(claims, audience=None, issuer=None, subject=None, algorithm=None, rbl_token=None, options=None):
    ...
    if options.get("verify_at_hash"):
        _validate_at_hash(claims, rbl_token, algorithm)
